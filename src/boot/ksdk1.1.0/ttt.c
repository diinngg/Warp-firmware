...
// learn from game, result = 0 for loss, 1 for draw, 2 for win
void learnFrom(const uint8_t result)
{
    ...
    for (; (moveNum < numMove) && (moveNum < 8); moveNum += 2)
    {
        ...
        if (lowNibble)
        {
            ...
            {
                case WIN:
                    if (boxWeight <= (15 - LEARN_INC))
                        *address += LEARN_INC;
                    else
                        *address += (15 - boxWeight);
                    break;
                case LOSS:
                    if (boxWeight > LEARN_INC)
                        *address -= LEARN_INC;
                    else if (boxWeight > 1)
                        *address -= (boxWeight - 1);
                    break;
